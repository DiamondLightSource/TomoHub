...
const SweepResultViewer: React.FC<SweepResultViewerProps> = ({
  ...
  const loadPage = useCallback(
    async (tiffUrl: string, pageIndex: number, centerValue: string) => {
  ...
  const preloadAdjacentPages = useCallback(
    async (tiffUrl: string, currentIndex: number, centerVals: string[]) => {
  ...
  const initializeTiffProcessing = async (tiffUrl: string) => {
    ...
    try {
      ...
      if (metadata.page_count < centerVals.length) {
        console.warn(
          `SweepResultViewer: TIFF has ${metadata.page_count} pages but expected ${centerVals.length}`
        );
      }
