import { Typography, Button, Box, ButtonGroup, Snackbar, Alert as MuiAlert } from '@mui/material';
          ...
          if (typeof content === 'string') {
            try {
              ...
              if (!Array.isArray(loadedData) || loadedData.length === 0) {
                setSnackbarState({ open: true, message: "Invalid YAML structure: Expected a HTTOMO YAML config file.", severity: "error" });
                return;
              }
              ...
              if (loaderConfig && loaderConfig.parameters) {
                ...
                if (params.rotation_angles?.data_path) {
                  loaderContext.setRotationAnglesDataPath(params.rotation_angles.data_path);
                ...
                if (params.darks) {
                  loaderContext.setDarks(params.darks.file || "", params.darks.data_path || "");
                }
                if (params.flats) {
                  loaderContext.setFlats(params.flats.file || "", params.flats.data_path || "");
                }
                
                loaderContext.removePreview(); // Clear existing
                if (params.preview?.detector_x) {
                  loaderContext.setDetectorX(params.preview.detector_x as PreviewType);
                }
                if (params.preview?.detector_y) {
                  loaderContext.setDetectorY(params.preview.detector_y as PreviewType);
                }
              } else {
                setSnackbarState({ open: true, message: "Warning: Loader configuration missing or invalid in YAML.", severity: "warning" });
              }

              // Process Methods Configuration
              const methodConfigs = loadedData.slice(1);
              const newMethods: MethodType[] = methodConfigs
                .filter(mc => !(mc.method === 'calculate_stats' && mc.id === 'statistics')) // Filter out auto-injected stats
                .map((mc: any) => ({
              ...
              setSnackbarState({ open: true, message: "Configuration loaded successfully!", severity: "success" });

            } catch (error) {
              console.error("Error parsing YAML or applying configuration:", error);
              setSnackbarState({ open: true, message: `Failed to load configuration: ${error instanceof Error ? error.message : "Unknown error"}`, severity: "error" });
            }
          }
        };
        reader.onerror = () => {
          setSnackbarState({ open: true, message: "Failed to read the file.", severity: "error" });
        };
        reader.readAsText(file);
      } else {
        setSnackbarState({ open: true, message: "Please select a YAML file (.yaml or .yml)", severity: "warning" });
      }
    }
